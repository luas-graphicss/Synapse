...
function fileIcon(path, isDir, open) {
  ...
  return window.SynapseFileIcons.render(path);
}
function colorOfExt(path) {
  return window.SynapseFileIcons.resolve(path).color;
}
...
async function filesFromEntries(entries) {
  ...
  await window.SynapseChunkedWork.forEachInChunks(entries, addEntry, {
      onProgress: (done, count) => setPreviewLoadingDetail(`Preparando ${done}/${count} arquivo(s)…`),
  });
  if (skipped) {
    try {
      toast(
...
function addProject(name, kind, files, opts) {
  ...
  const proj = window.SynapseProjectRecord.create({ id: nid(), name, kind, files, detect });
  proj.blocked =
  typeof auroraShouldBlock === 'function'
  ...
  const openFailures = window.SynapseGuardedSteps.runSteps([
      {
      ...
      {
        name: 'adoção pelo MCP',
        run: () => {
          if (typeof tmAdopt === 'function') tmAdopt(proj);
        },
      },
      {
        name: 'interface',
      ...
      },
      {
        name: 'editor',
        run: () => {
          if (activate && proj.openFile) openFileInEditor(proj.openFile);
        },
      },
      {
        name: 'preview',
        run: () => {
          if (activate) window.SynapsePreviewStart.startPreviewBuild(proj);
        },
      },
      {
        name: 'registro do projeto',
        run: () => {
  ...
  if (!activate) hidePreviewLoading();
  if (openFailures.length) window.SynapseProjectOpenReport.reportFailures(proj, openFailures);
  else toast('Importado', `“${name}” • ${files.size} arquivos`, 'ok');
  return proj;
}
...
async function handleDrop(dt) {
  ...
  try {
    ...
    if (plainFiles.length) {
      ...
      addProject('arquivos', 'folder', await filesFromEntries(ents));
      return;
    }
    hidePreviewLoading();
    setStatus('err', 'Falha');
...
async function importZipBytes(name, bytes) {
  ...
  setStatus('run', `Extraindo ${name}…`);
  await nextPaint();
  try {
    const entries = await parseZipAsync(bytes);
    setPreviewLoadingDetail(`Preparando ${entries.length} arquivo(s)…`);
    const files = await filesFromEntries(entries);
    addProject(name.replace(/\.zip$/i, ''), 'zip', files);
  } catch (err) {
    hidePreviewLoading();
    setStatus('err', 'Falha ao extrair');
    toast('Erro no .zip', err.message, 'err');
...
const treeNameCollator = new Intl.Collator();
let treeSearchActive = false;
function renderTree() {
  const proj = activeProject();
  ...
  treeSearchActive = el.exSearch.value.trim() !== '';
...
function renderDir(node, depth, proj) {
  const kids = [...node.children.values()].sort(
    (a, b) => b.dir - a.dir || treeNameCollator.compare(a.name, b.name),
  );
  let out = '';
  for (const k of kids) {
    const pad = 8 + depth * 13;
    if (k.dir) {
      const open = openDirs.has(k.path) || treeSearchActive;
...
function validRelPath(p) {
  ...
  if (/[<>:"|?*]/.test(p)) return false;
    ...
    function remapPaths(proj, fn) {
      window.SynapseExplorerSelection?.remap(proj, fn);
      window.SynapseDebug?.preferences.remap(proj, fn);
    ...
    async function promptRenameEntry(currentPath, isDirectory) {
      const project = activeProject();
      if (!project) return;
      const nextName = await uiPrompt(
        isDirectory ? 'Renomear pasta' : 'Renomear arquivo',
        'Digite apenas o novo nome. A pasta atual será mantida.',
        { placeholder: Core.basename(currentPath), okLabel: 'Renomear' },
      );
      if (nextName == null || !nextName.trim() || activeProject() !== project) return;
      if (typeof flushSnapshot === 'function') flushSnapshot();
      const result = window.SynapseRenamePath.renameEntry(
        project,
        currentPath,
        nextName,
        isDirectory,
      ...
      if (result.error) {
        ...
        ? 'O arquivo ou pasta não existe mais.'
        : 'Use apenas um nome, sem barras, ".." ou caracteres especiais.';
        toast('Não foi possível renomear', message, 'err');
        return;
      }
      if (!result.changed) return;
      toast('Renomeado', `"${currentPath}" → "${result.destinationPath}"`, 'ok');
      fsChanged(project, project.openFile);
    }
    async function ctxRenameFile(path) {
      return promptRenameEntry(path, false);
    }
    ...
    async function ctxRenameFolder(dir) {
      return promptRenameEntry(dir, true);
    }
    ...
    el.tree.addEventListener('contextmenu', (event) => window.SynapseExplorer?.contextMenu(event));
    ...
    el.tree.addEventListener('click', (event) => window.SynapseExplorer?.click(event));

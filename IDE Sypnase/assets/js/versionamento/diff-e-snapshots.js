...
function mcpAfterWrite(proj, path) {
  ...
  if (proj.id === State.active) {
    ...
    uiEscritaAgendar();
    if (window.MCP_ADMISSAO && window.MCP_ADMISSAO.ocupado()) proj.previewDirty = true;
    else scheduleBuild(proj);
  }
  if (typeof saveSessionSuave === 'function') saveSessionSuave();
  else saveSession();
}
...
const MCP_TOOLS = [
  {
    ...
    run: async (a) => {
      ...
      try {
        if (window.MCP_ADMISSAO && typeof window.MCP_ADMISSAO.resumo === 'function') {
          lines.push('');
          lines.push('Fila de chamadas nesta aba: ' + window.MCP_ADMISSAO.resumo());
          if (window.MCP_ADMISSAO.estado().saturada)
          lines.push(
            'ATENÇÃO: a fila está saturada. Envie as próximas chamadas uma a uma, esperando cada resposta.',
          );
        }
      } catch (e) {
        ignorarErro(e, 'project_status');
      }
      return lines.join('\n');
    },
  },
  ...
  },
  {
    ...
    run: async (a) => {
      ...
      const r = await visionCall(
        ...
        'reset',
        {
          keep_prefixes: ['aurora.'],
          keep_dbs: window.SynapseSessionStorageNames.preservedDatabaseNames(),
          cookies: !!a.cookies,
        },
        15000,
      );

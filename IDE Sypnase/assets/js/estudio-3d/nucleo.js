...
function est3dGLCtx() {
  ...
  const attempt = factory && factory.createContext(canvas, { preserveDrawingBuffer: true });
  const gl = attempt ? attempt.context : null;
  if (!gl) {
    throw new Error(
...
function MeshViewer(canvas, geo) {
  const factory = window.SynapseWebglContextFactory;
  const attempt = factory && factory.createContext(canvas);
  const gl = attempt ? attempt.context : null;
  ...
  let reducedMotion = mvReduceMotion();
  ...
  function render() {
    ...
    if (!projection || projectionAspect !== asp) {
      projection = m4perspective(
        (50 * Math.PI) / 180,
        asp,
        Math.max(0.001, geo.radius * 0.01),
        geo.radius * 200,
      );
      projectionAspect = asp;
    }
    const eye = [
  ...
  function loop() {
    raf = 0;
    if (!alive) return;
    if (!canObserveSize) sizeDirty = true;
    const spinning = auto && !dragging && !reducedMotion;
    if (spinning) {
      theta += 0.0045;
      needsRender = true;
    }
    ...
    if (spinning || !canObserveSize) raf = requestAnimationFrame(loop);
  }
  function down(e) {
    dragging = true;
    lx = e.clientX;
  ...
  const sizeObserver = canObserveSize ? new ResizeObserver(handleSizeChange) : null;
  if (sizeObserver) sizeObserver.observe(canvas);
  canvas.addEventListener('pointerdown', down);
  window.addEventListener('pointermove', move);
  window.addEventListener('pointerup', up);
  canvas.addEventListener('wheel', wheel, { passive: false });
  window.addEventListener('resize', handleSizeChange, { passive: true });
...
function openMediaPreview(proj, path, f) {
  ...
  if (is3DExt(ext)) {
    ...
    if (typeof mod3dAttachEditorAction === 'function') mod3dAttachEditorAction(bar, proj, path);

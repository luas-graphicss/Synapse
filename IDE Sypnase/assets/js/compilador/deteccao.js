(function (raiz, fabrica) {
    ...
    const AUXILIAR =
    /(^|\/)(tools?|scripts?|ferramentas|utilitarios|utils?|examples?|exemplos?|samples?|amostras|docs?|documentacao|tests?|testes|spec|specs|__tests__|benchmarks?|bench|native|nativo|nativos|addons?|plugins?|extras?|externals?|third[-_]?party|deps|dependencies|patches|ci|\.github|\.vscode|\.idea)(\/|$)/i;
    ...
    const MANIFESTO_NATIVO = ['cargo.toml', 'go.mod', 'pubspec.yaml', 'cmakelists.txt', 'meson.build'];
      ...
      { re: /#\s*include\s*[<"]SFML\//i, lib: 'SFML' },
        ...
        const GUI_NATIVA = [
          ...
          { re: /#\s*include\s*[<"]QtWidgets\//i, lib: 'Qt Widgets' },
            ...
            function nucleoDoProjeto(lista) {
              const dentro = [];
              for (let i = 0; i < lista.length; i++)
              if (!AUXILIAR.test(lista[i].caminho)) dentro.push(lista[i]);
              return dentro.length ? dentro : lista;
            }

            function entradaWeb(lista) {
              const html = porExt(lista, ['html', 'htm']);
              if (!html.length) return null;
              for (let i = 0; i < html.length; i++)
              if (/^index\.html?$/i.test(html[i].caminho)) return html[i];
              return achar(html, ['index.html', 'index.htm']) || html[0];
            }

            function indexNaRaiz(lista) {
              for (let i = 0; i < lista.length; i++)
              if (/^index\.html?$/i.test(lista[i].caminho)) return true;
              return false;
            }

            function montarWeb(lista) {
              ...
              if (pkg) {
                ...
                if (/"(vite|webpack|parcel|rollup|esbuild|next|nuxt)"/i.test(t)) precisaBundler = true;
                const mm = /"(react|vue|svelte|solid-js|preact)"/i.exec(t);
                if (mm) libsWeb.push(mm[1]);
              }
              const alvo = entradaWeb(lista);
              return resultado({
                  tipo: 'web',
                  nivel: precisaBundler ? 2 : 1,
                  gui: true,
                  entrada: alvo ? alvo.caminho : null,
            ...
            function webDomina(lista) {
              if (!entradaWeb(lista)) return false;
              if (achar(lista, MANIFESTO_NATIVO)) return false;
              if (porExt(lista, ['csproj', 'sln', 'fsproj']).length) return false;
              const nativa = porExt(lista, EXT_NATIVA).length;
              if (!nativa) return true;
              if (!indexNaRaiz(lista)) return false;
              return porExt(lista, EXT_WEB).length >= nativa * 6;
            }
            ...
            function detectar(arquivos) {
              ...
              const cmakeProject = globalThis.SYNAPSE_CMAKE_PROJECT?.primary(arquivos);
              if (cmakeProject) {
                return resultado({
                    tipo: 'cmake',
                    nivel: 2,
                    entrada: cmakeProject.manifest,
                    toolchain: 'cmake',
                    motivo: 'CMakeLists.txt encontrado',
                    avisos: ['Use o botão CMake para configurar, compilar e executar CTest pelo Relay local.'],
                });
              }
              const principal = nucleoDoProjeto(lista);
              if (webDomina(principal)) return montarWeb(principal);
              return detectarLinguagem(principal, !!entradaWeb(principal));
            }

            function detectarLinguagem(lista, temWeb) {
              if (achar(lista, ['cargo.toml'])) {
                ...
                if (csproj || cs.length) {
                  ...
                  const entradaCs = acharEntrada(
                    cs,
                    /static\s+(?:async\s+)?(?:void|int|Task)\s+Main\s*\(/,
                      ['program.cs'],
                    );
                    if (!csproj && !entradaConfirmada && temWeb) return montarWeb(lista);
                    if (wpf) {
                      return resultado({
                          tipo: 'csharp',
                          nivel: 0,
                    ...
                    return resultado({
                        ...
                        gui: avalonia,
                        entrada: entradaCs,
                        toolchain: avalonia ? 'avalonia-browser' : 'dotnet-relay',
                        ...
                        : [
                          'C# nao compila dentro do navegador: use Build & Run pelo Relay e importe a saida web completa.',
                        ],
                    });
                  }
                  ...
                  if (cpps.length || cs_.length) {
                    ...
                    const entrada = acharEntrada(
                      ...
                      /\bint\s+main\s*\(|\bvoid\s+main\s*\(/,
                        ...
                        if (!temMake && !entradaConfirmada && temWeb) return montarWeb(lista);
                        ...
                        return resultado({
                            ...
                            gui: false,
                            entrada: entrada,
                            toolchain: ehCpp ? 'clang-wasi' : 'wcc-wasi',
                            libs: [],
                            motivo: temMake
                      ...
                      if (achar(lista, ['go.mod'])) {
                        ...
                        if (py.length) {
                          const entradaPy = acharEntrada(py, /if\s+__name__\s*==/, [
                              'main.py',
                              'app.py',
                              '__main__.py',
                          ]);
                          if (!entradaConfirmada && temWeb) return montarWeb(lista);
                          return resultado({
                              tipo: 'python',
                              nivel: 1,
                              gui: false,
                        ...
                        if (porExt(lista, ['html', 'htm']).length || achar(lista, ['package.json'])) {
                          return montarWeb(lista);
                        }

...
function wireMcp() {
  ...
  if (!window.SynapsePreviewSession) mcpLoadCfg();
  const relayIn = $('#mcpRelay');
  if (relayIn) relayIn.value = window.SynapseRelayUrlPresentation.editableRelayValue(MCP.relay);
  ...
  let auto = false;
  try {
    auto = !window.SynapsePreviewSession && localStorage.getItem('aurora.mcp.active') === '1';
  } catch (e) {
    ignorarErro(e, 'wireMcp');
  }
  ...
  function wire() {
    ...
    $('#copyConsole').addEventListener('click', () => {
        const p = activeProject();
        const visible = p
        ? window.ConsoleLogFilter.visibleLogs(p.logs, State.consoleFilter, State.consoleSearch)
        : [];
        if (!visible.length) {
          toast('Console vazio', 'Nada para copiar neste filtro', '');
          return;
        }
        const txt = window.ConsoleLogFilter.toPlainText(visible, fmtLogTime);
        try {
          navigator.clipboard.writeText(txt);
          toast('Console copiado', visible.length + ' linha(s) copiada(s)', 'ok');
        } catch (e) {
    ...
    el.exSearch.addEventListener('input', window.SynapseFrameScheduler.create(renderTree));
    ...
    window.addEventListener('resize', window.SynapseFrameScheduler.create(applyDevice), { passive: true });
    window.addEventListener('message', (e) => {
        if (!(e.data && e.data.__lp_ready)) return;
        const mesmaOrigem = e.origin === window.location.origin || e.origin === 'null';
    ...
    wireMcp();
    wireTerm();
    el.qopenBtn.innerHTML = iconSvg('quickOpen');
    el.qopenBtn.addEventListener('click', () => openQuickOpen(true));
    if (el.fmtBtn) {
      el.fmtBtn.innerHTML = iconSvg('format');
      el.fmtBtn.addEventListener('click', formatActiveFile);
    }
    el.histBtn.innerHTML = iconSvg('history');
  ...
  function initResizer(handle, target, axis, min, max, flexGrow, invert) {
    ...
    let dragging = false;
    let pendingSize = 0;
    const applyPendingSize = window.SynapseFrameScheduler.create(() => {
        if (flexGrow) {
          ...
          target.style.height = pendingSize + 'px';
        }
        applyDevice();
    });
    function trackPointer(e) {
      ...
      let delta = (axis === 'w' ? e.clientX : e.clientY) - dragStartPosition;
      if (invert) delta = -delta;
      let nextSize = dragStartSize + delta;
      if (min) nextSize = Math.max(min, nextSize);
      if (max) nextSize = Math.min(max, nextSize);
    ...
    handle.addEventListener('mousedown', (e) => {
        if (dragging) return;
        dragging = true;
        dragStartPosition = axis === 'w' ? e.clientX : e.clientY;
        const rect = target.getBoundingClientRect();
        dragStartSize = axis === 'w' ? rect.width : rect.height;
        pendingSize = dragStartSize;
...
rig, animacoes, materiais e texturas intactos.`;
          ...
          try {
            MCP_INSTRUCTIONS +=
            ' BLEPSE 3D DO PROJETO: modeler_create_scene abre uma cena editavel no projeto; ' +
            'modeler_add_primitive adiciona cubo, esfera, cilindro, cone, plano, toro ou rampa; ' +
            'modeler_edit_mesh roda as operacoes de malha (extrudar, chanfrar, subdividir, cortar ' +
            'anel, espelhar, fundir); modeler_apply_material define cor, metalicidade, rugosidade ' +
            'e textura; modeler_get_scene devolve a arvore, a selecao e a revisao atual; ' +
            'modeler_export gera o arquivo .glb dentro do projeto. A ordem certa e: criar a cena, ' +
            'adicionar a primitiva, editar a malha, aplicar o material e exportar. Passe project e ' +
            'agent em todas as chamadas e repita o revision devolvido pela ultima escrita. Nada de ' +
            'upload nem download: os arquivos entram e saem pelo proprio projeto, e modeler_export ' +
            'conta como escrita, entao ele para no pedido de aprovacao nos modos Revisar e Estrito.';
          } catch (e) {
            ignorarErro(e, 'catalogo');
          }
          ...
          function appdLoop() {
            if (window.SynapsePreviewSession) return;

...
(function auroraCompact() {
    ...
    function proseFormatter() {
      return globalThis.SynapseAgentTextProse || null;
    }

    function firstSentence(text, max) {
      if (!text) return '';
      const formatter = proseFormatter();
      if (formatter) return formatter.firstSentence(text, max);
      const collapsed = String(text).replace(/\s+/g, ' ').trim();
      const sentenceMatch = collapsed.match(/^(.{20,}?)(?:\.\s|\;\s|\s\u2014\s)/);
    ...
    function clampDesc(text, max) {
      if (!text) return text;
      const formatter = proseFormatter();
      if (formatter) return formatter.clamp(text, max);
      const collapsed = String(text).replace(/\s+/g, ' ').trim();

...
(function auroraTelemetriaMcp() {
    ...
    function no(u) {
      try {
        ...
        .replace(/^https?:\/\//, '')
          .split('/')[0];
          return window.SynapseRelayUrlPresentation.maskRelayHost(host);
        } catch (e) {
          return String(u || '');
        }
      }
...
Se o Notion ainda ficar girando depois disto, o travamento e do lado do Notion.`
      ...
      if (typeof mcpEnviarResposta === 'function' && !mcpEnviarResposta.__telemetria) {
        ...
        const envTel = async function (reqId, out, packet) {
          const t0 = Date.now();
          try {
            const r = await _env(reqId, out, packet);
            if (VERBOSO)
            diga(
              'ok',
      ...
      function candidatas() {
        ...
        async function testar(opcoes) {
          ...
          try {
            ...
            diga(
              'ok',
              'URL entregue pelo painel: ' +
              window.SynapseRelayUrlPresentation.maskRelayHost(urlPublica()),
            );
...
pendurado. Correcao: atualize os nos e o portao para a versao 10 do servidor (o GET responde 405 na hora).`,
                );
                relatorio.push({
                    url: c.url,
                    rotulo: c.rotulo,
                    passo: 'GET (stream)',
                    resultado: 'GET pendurado - atualize o servidor para a v10',
                    ferramentas: nf,
                });
                continue;
              }
...
no Notion: ${bons[0].url} (ou rode SYNAPSE_MCP.usarMelhorUrl() para o painel passar a entregar ela).`,
        ...
        function usarMelhorUrl() {
          ...
          diga(
            'ok',
            `URL do painel trocada para ${no(m.base)}. Copie de novo e cole no Notion: ${window.SynapseRelayUrlPresentation.maskRelayHost(urlPublica())}`,
          );
          return urlPublica();
        }
          ...
          if (typeof _act === 'function' && !_act.__telemetria) {
            const act = function (silent) {
              ...
              try {
                ...
                diga(
                  'ok',
                  `MCP ativado. URL para o Notion: ${window.SynapseRelayUrlPresentation.maskRelayHost(urlPublica())} · vou narrar cada passo do handshake aqui.`,
                );
                const local =

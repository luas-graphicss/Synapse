...
function appAplicar(s) {
  if (!s || window.SynapsePreviewSession) return false;
  ...
  const relayFieldValue = window.SynapseRelayUrlPresentation.editableRelayValue(MCP.relay);
  if (ri && ri.value !== relayFieldValue) ri.value = relayFieldValue;
...
function mcpConnect() {
  ...
  es.addEventListener('rpc', (ev) => {
      ...
      for (const it of mcpPacotesRpc(pkt)) mcpOnRpc(it, responder);
  });
  es.onopen = () => {
...
async function mcpPollLoop() {
  ...
  const endpoint = mcpBase() + '/bridge/' + MCP.sid + '/' + MCP.token + '/poll';
  ...
  while (MCP.active && gen === MCP.pollGen && MCP.forcePoll) {
    try {
      ...
      if (MCP.active && gen === MCP.pollGen && j && Array.isArray(j.events)) {
        for (const ev of j.events) for (const it of mcpPacotesRpc(ev)) mcpOnRpc(it, responder);
      }

      ...
      function adotarNodos(info) {
        ...
        if (
          ...
          if (FLV.__avisoNodos !== trans) {
            ...
            logf(
              'err',
              `ATENCAO: o no desta aba (${nomeNo(trans)}) NAO esta na lista de nos do portao (${lista.map(nomeNo).join(', ')}). \
As chamadas podem cair num no sem catalogo e mostrar 0 ferramentas. Ajuste a lista de nos do portao \
ou abra o site com ?relay=${trans}`,
      ...
      async function provarPortao(forcar) {
        ...
        if (!antes || antes.ok !== novo.ok || (antes.servindo || '') !== (novo.servindo || '')) {
          ...
          logf(
            'err',
            `Portao sem resposta (${nomeNo(FLV.portao)}) — a URL unica pode estar fora; confira o endereco informado.`,
          );
      ...
      function captureReserveResponder(bridge, socket = bridge.ws) {
        return window.SynapseMcpReplyDelivery.createResponder({
            baseUrl: bridge.url,
            sessionId: MCP.sid,
            token: MCP.token,
            socket,
      ...
      async function rpcP(pt, pkt, resp = pt.responder || captureReserveResponder(pt)) {
        if (!pkt || pkt.reqId == null) return;
        if (typeof window.mcpExecutarPacote === 'function') {
          return await window.mcpExecutarPacote(pkt, resp);
        }
        let key = '';
        try {
          key = mcpReqKey(pkt);
        ...
        if (v) {
          if (v.done) resp(pkt.reqId, v.out, pkt);
          return;
        }
      ...
      function conectarP(pt) {
        ...
        pt.ws = ws;
        pt.responder = captureReserveResponder(pt, ws);
        ...
        const heartbeatTimer = setInterval(function () {
            if (pt.ws !== ws) {
              ...
              if (pt.pingT === heartbeatTimer) pt.pingT = null;
              return;
            }
            try {
        ...
        pt.pingT = heartbeatTimer;
      }
      async function pollP(pt, gen) {
        const responder = captureReserveResponder(pt, null);
        const endpoint = pt.url + '/bridge/' + MCP.sid + '/' + MCP.token + '/poll';
        if (pt.status !== 'online') {
          pt.status = 'connecting';
          pintar();
        }
        while (true) {
          ...
          try {
            const j = await window.SynapseMcpRequestJson(
              endpoint,
              {
                method: 'POST',
                headers: MCP_HDRS(),
            ...
            if (j && Array.isArray(j.events)) {
              for (let i = 0; i < j.events.length; i++) {
                ...
                for (let k = 0; k < pks.length; k++) rpcP(pt, pks[k], responder);
              }
            }
          } catch (e) {
            if (!ativoMcp() || pt.gen !== gen) return;
...
esta FORA DO AR — o encaminhamento interno depende do proprio no indisponivel. A sessao segue viva \
...
a sonda. Confira o endereco do portao. Emergencia: URL direta do no de reserva vivo: ${urlEmergencia() || '(nenhum no de reserva vivo)'}`;
          } else if (caido) {
            s = `🛑 <b>No principal indisponivel (provavel bloqueio de borda) — SEM portao, a URL unica do conector \
esta FORA DO AR.</b> O encaminhamento interno depende do proprio no indisponivel. A sessao \
esta preservada no no de reserva ${on ? on.i + 1 : 1}. Solucao definitiva: configure o <b>portao</b>\
 (menu MCP). Emergencia agora: ${urlEmergencia() || '(nenhum no de reserva vivo)'}`;
          } else {
        ...
        try {
          ...
          if (msg) {
            ...
            if (igual) {
              ...
              t =
              'Nenhum no de reserva configurado — failover desligado. Adicione enderecos acima para ativar o failover.';
            } else if (!ativoMcp()) {
              t =
              tot +
              ...
              if (on === tot && tot > 0) {
                ...
                t = 'Nos de reserva sem resposta — verifique os enderecos informados.';
                k = 'err';
              }
            }
          ...
          if (pmsg) {
            ...
            if (!FLV.portao) {
              tp =
              'Sem portao: o failover em URL unica cobre limite de capacidade, mas nao cobre a ' +
              'indisponibilidade do no principal (a URL cai junto). Informe o endereco do portao ' +
              'no campo acima.';
            } else if (!ativoMcp()) {
              ...
              tp = `Portao OK — failover completo em URL unica (cobre a queda do no principal)${pi.servindo ? ' · servindo por ' + nomeNo(pi.servindo) : ''}${pi.failover ? ' · FAILOVER ATIVO' : ''}.`;
              kp = 'ok';
            } else {
              tp = `Portao SEM RESPOSTA (${nomeNo(FLV.portao)}) — confira o endereco informado. Enquanto isso, a URL unica segue sem protecao extra.`;
              kp = 'err';
            }

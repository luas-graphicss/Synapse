...
(function auroraRelayWS() {
    ...
    function captureResponder(socket) {
    ...
    async function mcpOnRpcWS(pkt, responder = captureResponder(MCP.ws)) {
      if (!pkt || pkt.reqId == null) return;
      if (typeof window.mcpExecutarPacote === 'function') {
        return await window.mcpExecutarPacote(pkt, responder);
      }
      let out = null;
      try {
      ...
      await responder(pkt.reqId, out, pkt);
    }
    ...
    function mcpConnectWS() {
      ...
      const responder = captureResponder(ws);
      ws.onmessage = function (ev) {
        ...
        if (m.t === 'rpc') {
          ...
          if (typeof window.mcpDespacharPacotes === 'function') {
            window.mcpDespacharPacotes(pks, responder);
          } else {
            for (let pi = 0; pi < pks.length; pi++) mcpOnRpcWS(pks[pi], responder);
          }
          return;
        }
      };
      ...
      const heartbeatTimer = setInterval(function () {
          if (MCP.ws !== ws) {
            ...
            if (MCP.pingT === heartbeatTimer) MCP.pingT = null;
            return;
          }
          try {

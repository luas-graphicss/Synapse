...
(function auroraPoolWS() {
    ...
    function curto(u) {
      ...
      function alvos() {
        ...
        function responderDoSock(sock) {
          return (
            sock.responder ||
            window.SynapseMcpReplyDelivery.createResponder({
                baseUrl: sock.url || base(),
                sessionId: MCP.sid,
                token: MCP.token,
                socket: sock.ws,
            })
        ...
        async function executar(sock, pkt) {
          ...
          const resp = responderDoSock(sock);
        ...
        function tratarMsg(sock, ev) {
          ...
          if (m.t === 'rpc') {
            ...
            if (typeof window.mcpDespacharPacotes === 'function') {
              window.mcpDespacharPacotes(pks, responderDoSock(sock));
            } else {
        ...
        function conectar(sock, url) {
          ...
          sock.ws = ws;
          sock.responder = window.SynapseMcpReplyDelivery.createResponder({
              baseUrl: url,
              sessionId: MCP.sid,
              token: MCP.token,
              socket: ws,
        ...
        function sondar() {
          ...
          const alvo = {
            url,
            responder: window.SynapseMcpReplyDelivery.createResponder({
                baseUrl: url,
                sessionId: MCP.sid,
                token: MCP.token,
                socket: ws,
        ...
        async function canalExtra(gen) {
          const responder = window.SynapseMcpReplyDelivery.createResponder({
              baseUrl: base(),
              sessionId: MCP.sid,
              token: MCP.token,
          });
          const endpoint = base() + '/bridge/' + MCP.sid + '/' + MCP.token + '/poll';
          while (MCP.active && gen === MCP.pollGen && MCP.forcePoll) {
            try {
              ...
              if (MCP.active && gen === MCP.pollGen) {
                for (let a = 0; a < evs.length; a++) {
                  const pks = mcpPacotesRpc(evs[a]);
                  for (let b = 0; b < pks.length; b++) mcpOnRpc(pks[b], responder);
                }
              }

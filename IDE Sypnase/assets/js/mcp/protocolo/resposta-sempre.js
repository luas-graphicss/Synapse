...
(function auroraRespostaSempre() {
    ...
    async function mcpHandleMessageSempre(msg) {
      if (window.SynapseMcpCallLifecycle?.isManaged(msg)) return _orig(msg);
      const metodo = metodoDe(msg);
      ...
      const watchdog = new Promise((resolve) => {
          watchdogTimer = setTimeout(() => resolve({ tempo: 1 }), limite);
      });
      const r = await Promise.race([real, watchdog]).finally(() => clearTimeout(watchdogTimer));
    ...
    try {
      if (
        ...
        const enviarResposta = function (reqId, corpo) {
          try {
            const envio = mcpEnviarResposta(reqId, corpo);
            if (envio && typeof envio.catch === 'function')
            envio.catch(function (e) {
        ...
        const responderDepois = function (chave, reqId, idOriginal) {
          ...
          const tick = setInterval(function () {
              ...
              if (v && v.done) {
                ...
                let corpoPronto = v.out;
                if (v.omitido || corpoPronto == null) {
                  corpoPronto =
                  ...
                  : erroJson(
                    idOriginal,
                    -32000,
                    'Esta chamada ja foi executada nesta aba, mas a resposta era grande demais para ficar em cache. Nada foi executado de novo: confira o estado atual (project_status, list_files ou read_file) antes de repetir.',
                  );
                }
                enviarResposta(reqId, corpoPronto);
                return;
              }
              if (!v || fim) {
                clearInterval(tick);

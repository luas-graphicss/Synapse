...
async function mcpToolCall(params) {
  ...
  const executionName = window.SynapseMcpResolveToolName({ params: { name, arguments: args } });
  ...
  let __tmCtx = null;
  try {
    if (AG_WRITE.has(executionName)) {
      const agP = agProjQuiet(args);
      if (agP) agGate(agP, executionName, args);
    }
    __tmCtx = await tmBegin(executionName, args);
    const out = await tool.run(args);
    tmEnd(__tmCtx, true, null);
    __tmCtx = null;
    agTrack(agName(args), executionName, args, 1);
    ...
    agTrack(agName(args), executionName, args, 0);
...
async function headlessEnsure(proj) {
  ...
  if (!h) {
    ...
    ifr.setAttribute('allow', window.SynapsePreviewPermissions.allowValue());
    ifr.setAttribute(
      'sandbox',
...
async function visionCall(proj, action, args, timeoutMs) {
  ...
  const r = await new Promise((res, rej) => {
      ...
      try {
        ...
        if (frameOrigin && frameOrigin !== 'null') targetOrigin = frameOrigin;
      } catch (e) {
        ignorarErro(e, 'visionCall');
      }
      try {
        win.postMessage({ __lp_vision: true, id: id, action: action, args: a }, targetOrigin);
      } catch (e) {
        clearTimeout(timer);
        VIS.pending.delete(id);
        rej(e);
    ...
    if (ext === '.html' || ext === '.htm') {
      ...
      while ((m = re.exec(text))) {
        ...
        const ty = ((attrs.match(/\btype\s*=\s*["']?([^"'\s>]+)/i) || [])[1] || '').toLowerCase();
          ...
          function mcpHistMax() {
            try {
              const pm = window.SYNAPSE_MEM;
              if (pm && typeof pm.HIST_MAX === 'number' && pm.HIST_MAX > 0) return pm.HIST_MAX;
            } catch (e) {
              ignorarErro(e, 'mcpHistMax');
            }
            try {
              const p =
              State && State.projects ? State.projects.find((x) => x && x.id === State.active) : null;
          ...
          function mcpHist(f) {
            ...
            const max = mcpHistMax();
            if (f.history.length > max) f.history.splice(0, f.history.length - max);
            const TETO_HIST = 2097152;
            let bytes = 0;
            let manter = 0;
            for (let i = f.history.length - 1; i >= 0; i--) {
              const h = f.history[i];
              bytes += ((h && h.text && h.text.length) || 0) * 2;
              if (bytes > TETO_HIST && manter >= 1) break;
              manter++;
            }
            if (manter < f.history.length) f.history.splice(0, f.history.length - manter);
          }

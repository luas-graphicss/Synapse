(function () {
    ...
    const RE_TEMPLATE = /\$\{/g;
      ...
      function _nota() {
        const trocas = [`"${ANG}" no lugar de menor que`];
        if (NIVEL >= 2) {
          trocas.push(`"${RATIO}" no lugar de dois pontos em URIs javascript e data`);
          trocas.push(`"${FWEQ}" no lugar de igual em atributos on...`);
        }
        return [
          '[WAF] Resultado codificado para passar pela borda do Notion, que responde 403 a esse texto. Reversivel: ao escrever, mande o texto como preferir.',
          'Trocas: ' + trocas.join(' | ') + '.',
        ].join('\n');
      }

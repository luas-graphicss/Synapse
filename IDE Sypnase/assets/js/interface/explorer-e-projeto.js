          ...
          function baseSeguro(nome) {
            ...
            .replace(/[<>:"|?*\u0000-\u001f]/g, '_')
              ...
              function novoInline(tipo, dir) {
                ...
                abrirDirs(dir);
                PEND = { modo: 'novo', tipo: tipo, dir: dir, valor: '', projectId: proj().id };
                renderArvore();
                injetar();
              }
              ...
              function injetar() {
                ...
                inp.setAttribute(
                  'aria-label',
                  PEND.modo === 'rename' ? 'Novo nome' : 'Nome do arquivo ou pasta',
                );
              ...
              function confirmar(alvo, valBruto) {
                ...
                call('flushSnapshot');
                const renameResult = window.SynapseRenamePath.renameEntry(
                  p,
                  alvo.path,
                  String(valBruto),
                  alvo.tipo === 'dir',
                ...
                if (renameResult.error) {
                  const message =
                  renameResult.error === 'conflict'
                  ? 'Já existe um arquivo ou pasta com esse nome.'
                  : 'Use apenas um nome, sem barras, ".." ou caracteres especiais.';
                  if (renameResult.error === 'missing-source') {
                    av('Não foi possível renomear', 'O arquivo ou pasta não existe mais.', 'err');
                    renderArvore();
                    return;
                  }
                  return retomar(alvo, String(valBruto), ['Não foi possível renomear', message]);
                }
                if (!renameResult.changed) {
                  renderArvore();
                  return;
                }
                av('Renomeado', `"${alvo.path}" → "${renameResult.destinationPath}"`, 'ok');
                const refreshFileSystem = fn('fsChanged');
                if (refreshFileSystem) refreshFileSystem(p, p.openFile);
                else renderArvore();
              }
...
PC fraco: corta historico, minimapa e previews em segundo plano</small></button>`;
              ...
              function dirDoContexto() {
                if (window.SynapseExplorer) return window.SynapseExplorer.contextDirectory();
                const p = proj();
                if (!p) return '';
                const D = dirs(),
                d = pai(p.openFile || '');
              ...
              document.addEventListener(
                ...
                function (e) {
                  ...
                  if (e.target.closest?.('#tree') && window.SynapseExplorer) {
                    e.preventDefault();
                    e.stopPropagation();
                    window.SynapseExplorer.renameFocused();
                    return;
                  }

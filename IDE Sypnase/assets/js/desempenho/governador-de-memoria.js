(function () {
    ...
    const HEAP_LIMITE = (function () {
        try {
          const m = performance.memory;
          const lim = m && m.jsHeapSizeLimit;
          return typeof lim === 'number' && lim > 0 ? lim : 0;
        } catch (e) {
          return 0;
        }
    })();
    function pressaoAlta() {
      let usado = 0;
      try {
        usado = heap();
      } catch (e) {
      ...
      if (usado && HEAP_LIMITE && usado > HEAP_LIMITE * 0.55) return true;
      if (usado && usado > M.alvoMB * 1048576) return true;
      let n = 0;
      try {
        n = nArq();
      } catch (e) {
      ...
      if (n > M.grandeArq * 2) return true;
      try {
        const d = navigator.deviceMemory;
        if (typeof d === 'number' && d > 0 && d <= 2) return true;
      } catch (e) {
        ignorarErro(e, 'pressaoAlta');
      }
      return false;
    }
    const CH = (function () {
    ...
    function pausarPreview(auto) {
      const p = ativo();
      if (!p || pausado) return false;
      try {
        pausado = window.SynapsePreviewPause.pause(p);
      } catch (e) {
    ...
    function retomarPreview() {
      ...
      pausado = false;
      try {
        window.SynapsePreviewPause.resume(ativo());
      } catch (e) {
    ...
    function nivel1() {
      aplicarPM();
      try {
        const releaseModelerResources = fnw('mod3dReleaseApiResources');
        if (releaseModelerResources) releaseModelerResources();
      } catch (e) {
        ignorarErro(e, 'nivel1');
      }
      const r = {
    ...
    function ciclo() {
      ...
      try {
        ...
        M.heapMB = mb(h);
        if (!M.leve && !M.escolhaManual && (grande() || pressaoAlta())) modoLeve(true);
    ...
    function ligar() {
      ...
      else if (escolha !== '0' && (pressaoAlta() || CH <= 2)) M.leve = true;
      aplicarPM();
      envolverLogs();
      envolverSave();
      envolverEscritas();

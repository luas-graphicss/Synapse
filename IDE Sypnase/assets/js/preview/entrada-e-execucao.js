...
const AuroraFix = (function () {
    ...
    function looksLikeModule(code) {
      if (typeof validarSintaxe !== 'function') return false;
      const syntaxError = validarSintaxe(String(code || ''), { assincrono: false }) || '';
      return /Cannot use import statement outside a module|Cannot use 'import\.meta' outside a module|Unexpected token ['"]export['"]/.test(
        syntaxError,
      );
    }
                                            ...
                                            /(\bfrom\s*|\bimport\s*|\bexport\s*\*\s*from\s*|\bimport\()\s*(["'])([^"']+)\2/g,
                                              ...
                                              function resolveImportMapLocals(map, resolve, files, rawUrl, modules) {
                                                ...
                                                window.addEventListener('message', (e) => {
                                                    ...
                                                    const hprojC = isMainC
                                                    ? null
                                                    : window.SynapsePreviewRuntime.projectForSource(e.source) || headlessProjBySource(e.source);
                                                ...
                                                function renderConsole() {
                                                  ...
                                                  const q = (State.consoleSearch || '').trim();
                                                  const consoleFilter = window.ConsoleLogFilter;
                                                  el.cAll.textContent = proj.logs.length;
                                                  el.cErr.textContent = consoleFilter.countByCategory(proj.logs, 'error');
                                                  el.cWarn.textContent = consoleFilter.countByCategory(proj.logs, 'warn');
                                                  if (el.cMcp) el.cMcp.textContent = consoleFilter.countByCategory(proj.logs, 'mcp');
                                                  const logs = consoleFilter.visibleLogs(proj.logs, f, q);
                                                  if (!logs.length) {
                                                ...
                                                function openPopout() {
                                                  ...
                                                  w.document.open();
                                                  w.document.write(
                                                    POPOUT_SHELL.replace('__CH__', () => window.SynapsePreviewChannelName.forProject(proj.id))
                                                    .replace('__ORIGIN__', () => window.location.origin)
                                                    .replace('__NAME__', () => esc(proj.name)),
                                                  );
                                                  w.document.close();
                                                  proj.popout = w;
                                                  setTimeout(() => {
                                                ...
                                                const POPOUT_SHELL = [
                                                  ...
                                                  '<body><iframe id="f" referrerpolicy="no-referrer" allow="' +
                                                  window.SynapsePreviewPermissions.allowValue() +
                                                  '" sandbox="allow-scripts allow-same-origin allow-forms allow-popups ' +
                                                  'allow-pointer-lock allow-modals"></iframe>',
                                                  '<script>',
                                                  'var f=document.getElementById("f");',
                                                  'var presentation=(' +
                                                  window.SynapsePreviewFrames.create.toString() +
                                                  ' )({getFrame:function(){return f;},setFrame:function(next){f=next;}});',
                                                  'function set(h){return presentation.load(h);}',
                                                  'try{var ch=new BroadcastChannel("__CH__");ch.onmessage=function(e){if(e.data&&e.data.type==="html")set(e.data.html);};}catch(e){}',
                                                  'var EDITOR_ORIGIN="__ORIGIN__";',
                                                  'function isFromEditor(e){return e.origin===EDITOR_ORIGIN&&e.source&&e.source===window.opener;}',
                                                  'window.addEventListener("message",function(e){if(!isFromEditor(e))return;if(e.data&&e.data.__lp_popout)set(e.data.html);});',
...
<span class="tname">${esc(p.name)}</span><span class="tclose" data-close="${p.id}">${iconSvg('close')}</span>\
                                                ...
                                                function switchProject(id) {
                                                  window.SynapsePreviewRuntime.cancel();
                                                  State.active = id;
                                                  try {
                                                    headlessDestroy(id);
                                                  } catch (e) {
                                                ...
                                                function setDevice(dev) {
                                                  if (!Object.hasOwn(DEVICES, dev)) dev = 'responsive';
                                                  if (State.device !== dev) State.customDevice = null;
                                                  State.device = dev;
                                                  saveSession();
                                                  document.querySelectorAll('#deviceSeg button').forEach((button) => {
                                                      const selected = button.dataset.dev === dev;
                                                ...
                                                function applyDevice() {
                                                  ...
                                                  const dimensions = window.SynapseViewportGeometry.dimensions(
                                                    State,
                                                    DEVICES,
                                                    activeProject()?.viewport,
                                                  );
                                                  const zoom = document.getElementById('previewZoom');
                                                  if (zoom) zoom.hidden = !dimensions || State.layout === 'editor';
                                                  ...
                                                  const { width, height, border } = dimensions;
                                                  if (document.activeElement !== el.dimW) el.dimW.value = width;
                                                  if (document.activeElement !== el.dimH) el.dimH.value = height;
                                                  el.device.classList.toggle('bordered', border);
                                                  const scale = window.SynapseViewportGeometry.fit(
                                                    stage.width,
                                                    stage.height,
                                                    width,
                                                    height,
                                                  ...
                                                  if (scale !== null) placeDevice(width, height, scale, border ? 20 : 0);
                                                }
                                                function setCustomDims() {
                                                  if (!el.dimW.value || !el.dimH.value || !el.dimW.checkValidity() || !el.dimH.checkValidity())
                                                  return;
                                                  const width = Number(el.dimW.value);
                                                  const height = Number(el.dimH.value);
                                                  State.customW = State.rotated ? height : width;
                                                  State.customH = State.rotated ? width : height;
                                                  State.customDevice = State.device;
                                                  const project = activeProject();
                                                  if (project?.viewport) project.viewport = { ...project.viewport, w: width, h: height };
                                                  applyDevice();
                                                  saveSession();
                                                }
                                                function setLayout(lay) {
                                                  ...
                                                  document.querySelectorAll('#layoutSeg button').forEach((button) => {
                                                      const selected = button.dataset.lay === lay;
                                                      button.classList.toggle('on', selected);
                                                      button.setAttribute('aria-pressed', String(selected));
                                                  });
                                                  ...
                                                  if (lay === 'editor' && prevLay !== 'editor') {
                                                    window.SynapsePreviewRuntime.reset();
                                                    try {
                                                      el.frame.srcdoc = '';
                                                  ...
                                                  try {
                                                    ...
                                                    _wcListeners.push(
                                                      wc.on('server-ready', (port, url) => {
                                                          ...
                                                          el.frame.setAttribute('allow', window.SynapsePreviewPermissions.allowValue());
                                                          el.frame.src = url;
                                                ...
                                                function loadSample() {
                                                  const sample = window.SynapseSampleProject;
                                                  const encoder = new TextEncoder();
                                                  const files = new Map();
                                                  for (const file of sample.files)
                                                  files.set(file.path, makeFileEntry(file.path, encoder.encode(file.content)));
                                                  addProject(sample.name, 'folder', files);
                                                }

                                                const THEMES = [
                                                  { id: 'dark', name: 'Escuro', bg: '#11141b', acc: '#6aa3ff' },
                                                  { id: 'light', name: 'Claro', bg: '#ffffff', acc: '#6aa3ff' },
                                                  { id: 'solar', name: 'Solar', bg: '#fff8e7', acc: '#805800' },
...
style="background:${t.bg}"><i style="background:${t.acc}"></i></span><span>${t.name}</span></button>`,
                                                ...
                                                function sdb() {
                                                  ...
                                                  SDB.p = new Promise((res, rej) => {
                                                      let r;
                                                      try {
                                                        r = indexedDB.open(window.SynapseSessionStorageNames.databaseName(), 1);
                                                      } catch (e) {
                                                        return rej(e);
                                                      }
                                                      r.onupgradeneeded = () => {
                                                ...
                                                async function sdbPut(k, v) {
                                                  if (window.SynapsePreviewSession) return window.SynapsePreviewSession.set(k, v);
                                                  try {
                                                    const d = await sdb();
                                                ...
                                                async function sdbGet(k) {
                                                  if (window.SynapsePreviewSession) return window.SynapsePreviewSession.get(k);
                                                  try {
                                                    const d = await sdb();
                                                ...
                                                async function sdbDel(k) {
                                                  if (window.SynapsePreviewSession) return window.SynapsePreviewSession.delete(k);
                                                  try {
                                                    const d = await sdb();
                                                    await new Promise((res, rej) => {
                                                        const t = d.transaction('kv', 'readwrite');
                                                ...
                                                async function sdbKeys() {
                                                  if (window.SynapsePreviewSession) return window.SynapsePreviewSession.keys();
                                                  try {
                                                    const d = await sdb();
                                                    return await new Promise((res, rej) => {
                                                        const t = d.transaction('kv', 'readonly');
                                                ...
                                                function recSerProj(p) {
                                                  return {
                                                    ...
                                                    openFile: p.openFile,
                                                    openTabs: p.openTabs || [],
                                                    debuggerState: window.SynapseDebug?.preferences.sanitize(p.debuggerState) || null,
                                                ...
                                                function recRestoreProjData(pd, label) {
                                                  try {
                                                    ...
                                                    const proj = {
                                                      ...
                                                      kind: pd.kind || 'folder',
                                                      debuggerState: pd.debuggerState || null,
                                                      files,
                                                      detect,
                                                      entry: detect.entry,
                                                      openFile:
                                                    ...
                                                    try {
                                                      proj.channel = new BroadcastChannel(window.SynapsePreviewChannelName.forProject(proj.id));
                                                    } catch (e) {
                                                      ignorarErro(e, 'recRestoreProjData');
                                                    }
...
projetos</button></div>`;
                                                ...
                                                function _b64(buf) {
                                                  ...
                                                  const BLOCO = 32768;
                                                  if (b.length <= BLOCO) return btoa(String.fromCharCode.apply(null, b));
                                                  const partes = [];
                                                ...
                                                let __idleT = null;
                                                let idleLockArmedAt = 0;
                                                const IDLE_LOCK_REARM_INTERVAL = 5000;
                                                function resetIdleLock() {
                                                  ...
                                                  const now = Date.now();
                                                  if (__idleT && now - idleLockArmedAt < IDLE_LOCK_REARM_INTERVAL) return;
                                                  clearTimeout(__idleT);
                                                  idleLockArmedAt = now;
                                                  __idleT = setTimeout(() => {
                                                      __idleT = null;
                                                ...
                                                function serializeSession() {
                                                  return {
                                                    ...
                                                    customDevice: State.customDevice || null,
                                                ...
                                                function restoreSession(data) {
                                                  ...
                                                  for (const pd of data.projects) {
                                                    try {
                                                      ...
                                                      : detect.type === 'build' && !(window.crossOriginIsolated && navigator.onLine);
                                                      try {
                                                        proj.channel = new BroadcastChannel(
                                                          window.SynapsePreviewChannelName.forProject(proj.id),
                                                        );
                                                      } catch (e) {
                                                        ignorarErro(e, 'restoreSession');
                                                      }
                                                  ...
                                                  if (
                                                    [data.customW, data.customH].every(
                                                      (value) => Number.isInteger(value) && value >= 16 && value <= 8192,
                                                    )
                                                  ) {
                                                    State.customW = data.customW;
                                                    State.customH = data.customH;
                                                    State.customDevice = data.customDevice === State.device ? data.customDevice : null;
                                                  }
                                                  State.rotated = data.rotated === true;
                                                  State.suppressSave = false;
                                                  return true;
                                                }
                                                ...
                                                function commandList() {
                                                  ...
                                                  if (window.SYNAPSE_BUILD) cmds.push(...window.SYNAPSE_BUILD.commands());
                                                ...
                                                function renderPalette(q) {
                                                  ...
                                                  const paletteFilter = window.SynapseCommandPaletteFilter;
                                                  const items = paletteFilter ? paletteFilter.filterCommands(all, q) : all;
                                                  __cmd.items = items;
                                                  __cmd.sel = 0;
...
<span>${relTime(v.t)}${v.cur ? ' • não salva' : ''}</span></button>`,
                                                    ...
                                                    function closeOverlays() {
                                                      ...
                                                      el.histBack.classList.add('hidden');
                                                      if (el.frBack && !el.frBack.classList.contains('hidden')) openFindReplace(false);
                                                      State.paletteOpen = false;
                                                    }
                                                    function runWorkspaceShortcut(commandId) {
                                                      if (commandId === 'workspace.commandPalette') {
                                                        const open = !el.cmdkBack.classList.contains('hidden');
                                                        closeOverlays();
                                                        if (!open) openPalette(true);
                                                        return true;
                                                      }
                                                      if (commandId === 'workspace.quickOpen') {
                                                        const open = !el.qopenBack.classList.contains('hidden');
                                                        closeOverlays();
                                                        if (!open) openQuickOpen(true);
                                                        return true;
                                                      }
                                                      if (commandId === 'workspace.history') {
                                                        const open = !el.histBack.classList.contains('hidden');
                                                        closeOverlays();
                                                        if (!open) openHistory();
                                                        return true;
                                                      }
                                                      if (commandId === 'editor.findInFile') {
                                                        closeOverlays();
                                                        openFindReplace(true, { scope: 'file' });
                                                        return true;
                                                      }
                                                      if (commandId === 'editor.findInProject') {
                                                        const open = !el.frBack.classList.contains('hidden');
                                                        closeOverlays();
                                                        if (!open) openFindReplace(true);
                                                        return true;
                                                      }
                                                      if (commandId === 'editor.replaceInProject') {
                                                        closeOverlays();
                                                        openFindReplace(true, { scope: 'project', replace: true });
                                                        return true;
                                                      }
                                                      if (commandId === 'editor.formatFile') {
                                                        formatActiveFile();
                                                        return true;
                                                      }
                                                      if (commandId === 'panels.refreshPreview') {
                                                        const project = activeProject();
                                                        if (project) {
                                                      ...
                                                      if (commandId === 'workspace.toggleSidebar') {
                                                        toggleSidebar();
                                                        return true;
                                                      }
                                                      if (commandId === 'panels.toggleConsole') {
                                                        openConsole(!State.consoleOpen);
                                                        return true;
                                                      }
                                                      if (commandId === 'layout.split') {
                                                        setLayout('split');
                                                        return true;
                                                      }
                                                      if (commandId === 'layout.editor') {
                                                        setLayout('editor');
                                                        return true;
                                                      }
                                                      if (commandId === 'layout.preview') {
                                                        setLayout('preview');
                                                        return true;
                                                      }
                                                      return false;
                                                    ...
                                                    function handleShortcut(e) {
                                                      ...
                                                      const dispatcher = window.SynapseShortcutDispatcher;
                                                      if (!dispatcher) return;
                                                      const shortcut = dispatcher.handle(e);
                                                      if (!shortcut) return;
                                                      if (State.paletteOpen && !shortcut.worksWithOverlay) return;
                                                      if (runWorkspaceShortcut(shortcut.id)) {
                                                        e.preventDefault();
                                                        return;
                                                      }
                                                      if (dispatcher.runOwnedCommand(shortcut)) e.preventDefault();
                                                    }

                                                    ICONS.mcp =
                                                    '<path d="M9 2v5M15 2v5"/><path d="M6 7h12v4a6 6 0 0 1-12 0z"/><path d="M12 17v2.5a2.5 2.5 0 0 1-2.5 2.5H7"/>';

                                                    const MCP_RELAY_PADRAO = window.SynapseRelayDefaultEndpoint.defaultRelayOrigin();
                                                    ...
                                                    function mcpLoadCfg() {
                                                      try {
                                                        MCP.relay = window.SynapseRelayUrlPolicy.safeRelayUrl(localStorage.getItem('aurora.mcp.relay'));
                                                        MCP.pub = window.SynapseRelayUrlPolicy.safeRelayUrl(localStorage.getItem('aurora.mcp.pub'));

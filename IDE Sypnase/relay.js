...
function decodeOemBytes(buffer) {
  let texto = '';
  for (let indice = 0; indice < buffer.length; indice++) {
    const byte = buffer[indice];
    texto += byte < 128 ? String.fromCharCode(byte) : OEM_HIGH_RANGE[byte - 128];
  }
  return texto;
}

function completeUtf8Length(buffer) {
  const maximoDaSequencia = 4;
  let cursor = buffer.length;
  while (cursor > 0 && buffer.length - cursor < maximoDaSequencia) {
    const byte = buffer[cursor - 1];
    if (byte < 0x80) return buffer.length;
    if (byte >= 0xc0) {
      const necessarios = byte >= 0xf0 ? 4 : byte >= 0xe0 ? 3 : 2;
      const disponiveis = buffer.length - (cursor - 1);
      return disponiveis >= necessarios ? buffer.length : cursor - 1;
    }
    cursor--;
  }
...
function createProcessTextDecoder() {
  ...
  function decodificar(bytes) {
    ...
    try {
      return utf8Estrito.decode(bytes);
    } catch (erro) {
      return decodeOemBytes(bytes);
    }
  }
  return {
    write(pedaco) {
      pendente = pendente.length ? Buffer.concat([pendente, pedaco]) : Buffer.from(pedaco);
      const prontos = completeUtf8Length(pendente);
      if (!prontos) return '';
      const bytes = pendente.subarray(0, prontos);
      const resto = Buffer.from(pendente.subarray(prontos));
...
const TIMEOUT_MS = Number(process.env.AURORA_TIMEOUT_MS || 35000);
const VERSION = 'v9.8.1-mcp-resilience';
...
function cors(res) {
  ...
  res.setHeader(
    ...
    'Content-Type, Authorization, Accept, Notion-Version, Mcp-Session-Id, MCP-Protocol-Version, Last-Event-ID, ngrok-skip-browser-warning, X-Requested-With, Cache-Control, Pragma',
  );
...
const MAX_PENDING = Number(process.env.AURORA_MAX_PENDING || 256);
const PER_WORKER = Number(process.env.AURORA_PER_WORKER || 64);
const GRACE_MS = Number(process.env.AURORA_GRACE_MS || 8000);
const CLIENT_TTL = Number(process.env.AURORA_CLIENT_TTL || 900000);
const QUEUE_MAX = Number(process.env.AURORA_QUEUE_MAX || 256);
...
function wafSanearTexto(s) {
  ...
  function getSession(sid) {
    ...
    if (!s) {
      s = {
        ...
        lastSeen: 0,
        seq: 1,
        epoch: crypto.randomBytes(12).toString('hex'),
        wseq: 1,
        work: {},
        procs: new Map(),
        procSeq: 1,
  ...
  function vivoW(w) {
    ...
    if (w.kind === 'sse') return !!(w.res && !w.res.destroyed && !w.res.writableEnded);
    return false;
  }
  function workersVivos(session) {
    const alive = [];
    for (const worker of session.workers) {
      ...
      worker.morto = true;
      for (const requestId of worker.inflight || []) {
        const pending = session.pending.get(requestId);
        if (!pending || pending.worker !== worker) continue;
        pending.worker = null;
        if (!session.queue.some((item) => item.reqId === requestId))
        session.queue.push({ reqId: requestId, body: pending.body });
      }
      worker.inflight?.clear();
    }
    session.workers = alive;
    return alive;
  }
  ...
  function flushPoll(s) {
    while (s.polls.length && s.queue.length) {
      ...
      .splice(0, PER_WORKER)
      .filter(
        (item) => s.pending.has(item.reqId) && s.pending.get(item.reqId).expiresAt > Date.now(),
      )
      .map((item) => ({
            ...item,
            ep: s.epoch,
            budgetMs: Math.max(0, s.pending.get(item.reqId).expiresAt - Date.now()),
      }));
  ...
  function bombear(s) {
    while (s.queue.length) {
      ...
      if (!p || p.expiresAt <= Date.now()) continue;
      const packet = { ...obj, ep: s.epoch, budgetMs: Math.max(0, p.expiresAt - Date.now()) };
      if (!enviarW(w, JSON.stringify({ t: 'rpc', ...packet, items: [packet] }))) {
        s.queue.unshift(obj);
        w.morto = true;
        workersVivos(s);
        continue;
  ...
  function entregarItens(s, itens, worker) {
    ...
    for (let i = 0; i < itens.length; i++) {
      ...
      if (!p || (itm.ep != null && itm.ep !== s.epoch)) {
        exp++;
        expiredIds.push(itm.reqId);
        continue;
      }
      s.pending.delete(itm.reqId);
      s.queue = s.queue.filter((item) => item.reqId !== itm.reqId);
    ...
    s.lastSeen = Date.now();
    bombear(s);
    return { delivered: ent, expired: exp, expiredIds };
  }
  ...
  function waitForFileRetry(milliseconds) {
  ...
  async function writeFileRetry(abs, buf) {
    for (let a = 0; ; a++) {
      try {
        ...
        await waitForFileRetry(50 * (a + 1));
      }
    }
  }
  async function unlinkRetry(p) {
    for (let a = 0; ; a++) {
      try {
    ...
    for (let i = 0; i < segs.length; i++) {
      ...
      async function scanDir(root) {
        ...
        while (stack.length) {
          ...
          let ents;
          try {
            ents = await fs.promises.readdir(abs, { withFileTypes: true });
          } catch (e) {
            out.parcial = true;
            continue;
          }
      ...
      const diskActions = new Map();
      let pendingDiskActions = 0;

      async function scheduleDiskAction(session, action, body, response) {
        if (!['sync', 'changes'].includes(action)) return termAction(session, action, body, response);
        const project = safeName(body.project);
        const previous = diskActions.get(project);
        if (pendingDiskActions >= 16 || (previous?.count || 0) >= 4) {
          sendJson(response, 429, {
              error: 'Disk queue capacity reached. Nothing was executed.',
              executed: false,
              retry_after: 1,
          });
          return;
        }
        const state = previous || { tail: Promise.resolve(), count: 0 };
        state.count++;
        pendingDiskActions++;
        diskActions.set(project, state);
        const execution = state.tail
        .catch(() => {})
        .then(() => termAction(session, action, body, response));
        ...
        try {
          ...
          state.count--;
          pendingDiskActions--;
          if (!state.count) diskActions.delete(project);
        }
      }

      async function termAction(s, act, b, res) {
        if (act === 'sync') {
          ...
          if (b.prune !== false) {
            ...
            for (let j = 0; j < keys.length; j++) {
              ...
              if (await unlinkRetry(absFor(dir, k))) {
                deleted++;
                delete man[k];
              }
            }
        ...
        if (act === 'run') {
          ...
          const decOut = createProcessTextDecoder(),
          decErr = createProcessTextDecoder();
        ...
        if (act === 'listdisk') {
          ...
          let ents2;
          try {
            ents2 = await fs.promises.readdir(WORK, { withFileTypes: true });
          } catch (e) {
            ents2 = [];
          }
          for (let d2 = 0; d2 < ents2.length; d2++) {
            ...
            let cnt = 0;
            try {
              cnt = (await scanDir(path.join(WORK, en2.name))).size;
            } catch (e) {
              cnt = 0;
            }
      ...
      function proxyNotion(req, res, u, parts) {
        const desloc = parts[0] === 'api' ? 2 : 1;
        const caminho = '/' + parts.slice(desloc).join('/');
        if (caminho.indexOf('/v1/') !== 0) {
          sendJson(res, 400, { error: 'use /notion/v1/...' });
          return;
        }
        if (typeof fetch !== 'function') {
          sendJson(res, 500, { error: 'proxy do Notion requer Node 18 ou superior' });
          return;
        }
        const hN = {
          'notion-version': String(req.headers['notion-version'] || '2026-03-11'),
          'content-type': String(req.headers['content-type'] || 'application/json'),
          accept: 'application/json',
        };
        if (req.headers.authorization) hN.authorization = String(req.headers.authorization);
        const pedacos = [];
        req.on('data', function (d) {
            pedacos.push(d);
        });
        req.on('end', function () {
            const met = String(req.method || 'GET').toUpperCase();
            const corpoN =
            met === 'GET' || met === 'HEAD' ? undefined : Buffer.concat(pedacos).toString('utf8');
            const ctlN = new AbortController();
            const tmoN = setTimeout(function () {
                try {
                  ctlN.abort();
                } catch (eN) {
                  ignorarErro(eN, 'proxyNotion');
                }
              }, 60000);
            fetch('https://api.notion.com' + caminho + (u.search || ''), {
                method: met,
                headers: hN,
                body: corpoN,
                signal: ctlN.signal,
            ...
            .then(function (rN) {
                return rN.text().then(function (txN) {
                    cors(res);
                    res.writeHead(rN.status, {
                        'Content-Type': rN.headers.get('content-type') || 'application/json; charset=utf-8',
                        'Cache-Control': 'no-store',
                    });
                    res.end(txN);
                });
            ...
            .catch(function (eN2) {
                sendJson(res, 502, {
                    error: 'proxy do Notion falhou: ' + ((eN2 && eN2.message) || String(eN2)),
                });
            })
            .finally(function () {
                clearTimeout(tmoN);
      ...
      const server = http.createServer(function (req, res) {
          ...
          if (parts[0] === 'notion' || (parts[0] === 'api' && parts[1] === 'notion')) {
            proxyNotion(req, res, u, parts);
            return;
          }
          ...
          if (parts[0] === 'bridge' && parts.length === 4 && parts[3] === 'poll' && req.method === 'POST') {
            ...
            .then(function (raw) {
                ...
                if (s5.queue.length) {
                  ...
                  .filter(
                    (item) =>
                    s5.pending.has(item.reqId) && s5.pending.get(item.reqId).expiresAt > Date.now(),
                  )
                  .map((item) => ({
                        ...item,
                        ep: s5.epoch,
                        budgetMs: Math.max(0, s5.pending.get(item.reqId).expiresAt - Date.now()),
                  }));
                  for (let i = 0; i < lote.length; i++) {
          ...
          if (
            ...
            .then(function (raw) {
                ...
                sendJson(res, 200, { ok: true, ep: s2.epoch, ...r9 });
            })
            .catch(function (e) {
                sendJson(res, 400, { error: String((e && e.message) || e) });
            });
          ...
          if (
            ...
            if (!sessions.has(parts[1])) {
              sendJson(res, 403, { error: 'sessao desconhecida para terminal/disco' });
              return;
            }
            ...
            .then(function (raw) {
                ...
                return scheduleDiskAction(sT, act, b || {}, res).catch((error) => {
            ...
            .then(function (raw) {
                ...
                pend.timer = setTimeout(
                  function () {
                    ...
                    sM.queue = sM.queue.filter((item) => item.reqId !== reqId);
      ...
      server.on('upgrade', function (req, sock, head) {
          ...
          wsAttach(
            ...
            function (txt) {
              ...
              wsSend(sock, JSON.stringify({ t: 'ack', ep: sw.epoch, ids: ids, ...r }));
            },
            function () {
              clearInterval(hbw);
              removeWorker(sw, w);
